...
infiles = glob.glob("*random_partition.txt") #I'm literally just using this for the names--there's an equal number of partition and constrauned_structures files 

nameregex = r"(^[a-zA-Z]\d+)|(^[a-zA-Z]+)"
bsuregex = r"\d+"

for infile in infiles:
	nametuple = re.findall(nameregex, infile)[0]
	...
	if name == "BSU": #for genes of the name BSU_misc_RNA_3 or something--we want to isolate the 3
		bsunumber = re.findall(bsuregex, infile)[0]
	...
	if name == "Random": #random sequences are named something like Random_sequence_3600_random.txt--we want to isolate the 3600
		randomnumber = re.findall(bsuregex, infile)[0]
		name = "Random_sequence_"+randomnumber


	unconstrained = open(name+"_random_partition.txt", "r") #RNAfold -p0 output for sequence without constraints
	constrained = open(name+"_random_constrained_structures.txt", "r") #RNAfold -p0 -C output for sequences with a single nt constrained to be unpaired
	
	#check if the datfile already exists so you don't append to it
	datfile = name+"_random_paired_probs.dat"
	if path.exists(datfile):
		print datfile 
		unconstrained.close()
		constrained.close()
		continue #if yes, advance to the next loop iter
	
	else: 
		probabilities = open(name+"_random_paired_probs.dat", "w") #This is the .dat file you'll be outputting the paired probabilities to 
